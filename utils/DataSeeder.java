...
/** Seeds demo data on first startup when no data files exist. */
public class DataSeeder {

    /** Seeds data when the system has no usable records. Always seeds staff accounts. */
    public static boolean seedIfEmpty() {
        seedStaffAccounts();

        // Check actual loaded data, not just file existence — old .dat files from
        // previous versions may exist but fail to deserialise (returning empty lists)
    ...
    private static List<UserAccount> createDemoStudents(List<LibraryItem> items) {
        ...
        // s1: two books, one overdue
        borrowForStudent(s1, items.get(0), LocalDate.now().minusDays(20));
        borrowForStudent(s1, items.get(1), LocalDate.now().minusDays(5));

        // s2: one magazine
        borrowForStudent(s2, items.get(15), LocalDate.now().minusDays(3));

        // s3: one journal, overdue
        borrowForStudent(s3, items.get(20), LocalDate.now().minusDays(18));
        ...
        // s6-s8: no active loans but have return history
        ...
        students.add(s7);  students.add(s8);  students.add(s9);
        students.add(s10);
    ...
    private static Magazine makeMagazine(String id, String title, String author, int year, int copies) {
        ...
        m.setTotalCopies(copies); m.setAvailableCopies(copies);
        return m;
    }
    ...
    private static Journal makeJournal(String id, String title, String author, int year, int copies) {
        ...
        j.setTotalCopies(copies); j.setAvailableCopies(copies);
        return j;
    }

    // Creates an active loan and decrements the item's available copies
    private static void borrowForStudent(UserAccount student, LibraryItem item, LocalDate borrowDate) {
        if (item instanceof Borrowable && item.getAvailableCopies() > 0) {
            item.setAvailableCopies(item.getAvailableCopies() - 1);
            student.getCurrentLoans().add(new BorrowRecord(item, borrowDate));
        }
    }

    // Creates a completed loan directly in the student's history
